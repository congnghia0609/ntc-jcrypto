...
public class TestSSS {
    ...
    public void testEncodeDecode() {
        ...
        Assert.assertEquals("Encode Base64", "lOpFwywpCeVAcK0_LOKG-YtW71xyj1bX06CcW7VZMFc=", b64data);
        String hexdata = sss.toHex(number);
        Assert.assertEquals("Encode Hex", "94ea45c32c2909e54070ad3f2ce286f98b56ef5c728f56d7d3a09c5bb5593057", hexdata);
        BigInteger numb64decode = sss.fromBase64(b64data);
        Assert.assertEquals("Decode Base64", 0, numb64decode.compareTo(number));
        BigInteger numhexdecode = sss.fromHex(hexdata);
        Assert.assertEquals("Decode Hex", 0, numhexdecode.compareTo(number));
    }
    ...
    public void testSplitMerge() {
        ...
        List<BigInteger> arr = sss.splitSecretToBigInt(s);
        Assert.assertEquals("inNumbers", true, sss.inNumbers(arr, new BigInteger("49937119214509114343548691117920141602615245118674498473442528546336026425464")));
        String r = sss.mergeBigIntToString(arr);
        Assert.assertEquals("Split Merge", s, r);
    }
 
    @Test
    public void testFullBase64() {
        try {
            String s = "nghiatcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
    ...
    public void testBase64Combine() {
        try {
            ...
            String s1 = sss.combine(arr.subList(0, 3), true);
            Assert.assertEquals("Base64Combine combine 1", s, s1);

            String s2 = sss.combine(arr.subList(3, 6), true);
            Assert.assertEquals("Base64Combine combine 2", s, s2);

            String s3 = sss.combine(arr.subList(1, 5), true);
            Assert.assertEquals("Base64Combine combine 3", s, s3);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void testHexCombine() {
        try {
            ...
            String s1 = sss.combine(arr.subList(0, 3), false);
            Assert.assertEquals("HexCombine combine 1", s, s1);

            String s2 = sss.combine(arr.subList(3, 6), false);
            Assert.assertEquals("HexCombine combine 2", s, s2);

            String s3 = sss.combine(arr.subList(1, 5), false);
            Assert.assertEquals("HexCombine combine 3", s, s3);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
